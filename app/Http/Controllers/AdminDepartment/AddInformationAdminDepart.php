...
class AddInformationAdminDepart extends Controller
{
    public function index()
        ...
        public function addInformationAdminDepartment(Request $request, $id)
        {
 ...
 if(!(Hash::check($request->get('pass_old'), Auth::guard('department')->user()->password))) {
  ...
  if($request->get('pass_new') != $request->get('confirm_pass_new')) {
    return redirect()->back()->with("error5", "Mật khẩu nhập lại không trùng khớp!");
            ...
            return redirect('/admin-department')->with("success1", 'Cập nhật thành công!');
            }
            else{
